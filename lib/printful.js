...
export async function resolveCatalogVariant(productId, sizeLabel) {
  ...
  if (!v) {
    // One-size products can still expose multiple variants (e.g. a tote sold in a
    // single size across several colors), so a label like "Standard" matches no
    // variant size. When there's only one distinct size, fall back to the first
    // variant rather than failing to resolve.
    const distinctSizes = new Set(variants.map(x => x.size).filter(Boolean));
    if (variants.length === 1 || distinctSizes.size <= 1) v = variants[0];
  }
  if (!v) {
