...
// unlocked only for paid ordering tiers (collector/patron), resolved server-side
// from the authoritative /api/user-tier endpoint (backed by NextAuth + Neon). The
// old exported names (useShopGate / PinModal / TradeAccessPanel) are kept so the
...
// NOTE: must mirror PAID_TIERS in lib/authdb.js. Explorer is a paid browse tier
// but does NOT include ordering, so it is intentionally excluded here.
const PAID_TIERS = new Set(['collector', 'patron']);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '20px 12px', textAlign: 'center' }}>
      ...
      <p style={{ fontSize: 13, color: '#8A8178', marginBottom: 20, lineHeight: 1.6, maxWidth: 300 }}>
        {signedIn
          ? 'Upgrade to a Collector or Patron plan to order prints, canvas, and more of this work.'
