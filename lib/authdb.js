...
export function tierDiscount(tier) {
  if (tier === 'patron') return 0.20;
  if (tier === 'collector') return 0.10;
  return 0; // explorer (browse-only) and free get no order discount
}

// Tiers that unlock print ordering. Explorer is a paid browse/search tier but
// does NOT include ordering — that starts at Collector (see pricing page).
export const PAID_TIERS = new Set(['collector', 'patron']);
