...
export function hasSubscriptionPrices() {
  return !!(process.env.STRIPE_PRICE_EXPLORER && process.env.STRIPE_PRICE_COLLECTOR && process.env.STRIPE_PRICE_PATRON);
}
...
export async function createCheckoutSession({ priceId, customerEmail, clientReferenceId, successUrl, cancelUrl, metadata = {} }) {
  const form = {
    ...
    // Collect billing address so the webhook can attribute giving by location.
...
// Hosted Customer Billing Portal — lets a subscriber update payment method,
// view invoices, or cancel. Returns { url }. Requires the portal to be enabled
