...
// Trade-access PIN gating the order UI. Must match pages/viewer.js (same value + the
// shared sessionStorage key 'shopUnlocked' so one unlock covers both pages).
// NOTE: client-side soft gate only — the value ships in the browser bundle.
const SHOP_PIN = '2025';
...
export default function Home() {
  ...
  // Restore trade-access unlock for the session (shared with /viewer).
  useEffect(() => {
        ...
        <div className="order-banner">
          ...
          <div className="order-banner-actions">
            ...
            {shopUnlocked ? (
              ...
              <button className="order-confirm" onClick={() => setShowPinModal(true)}>🔒 Enter PIN to order</button>
            )}
          </div>
