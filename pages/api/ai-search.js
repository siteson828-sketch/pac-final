...
export default async function handler(req, res) {
  ...
  try {
    ...
    const strict = ((mustRe || artistGated) && reqMode !== 'broad') ? await sql`
      ...
      ORDER BY ROW_NUMBER() OVER (PARTITION BY lower(trim(regexp_replace(split_part(coalesce(artist,''), ${NL}, 1), ${STRIP}, ''))) ORDER BY score DESC, synced_at DESC), score DESC, synced_at DESC
