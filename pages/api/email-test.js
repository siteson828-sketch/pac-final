...
// welcome template and sends it FROM the production sender (RESEND_FROM_EMAIL) by
// default, to confirm the verified domain sends to real recipients. Remove after.
export default async function handler(req, res) {
  ...
  const from = String(req.query.from || process.env.RESEND_FROM_EMAIL || 'Public Art Collections <hello@publicartcollections.net>').slice(0, 160).trim();
