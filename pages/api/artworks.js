...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { search, source, limit = '24', offset = '0', count } = req.query;
      ...
      works = await sql`
        ...
        WHERE commercial_ok = true AND thumb_url IS NOT NULL AND thumb_url != ''
          ...
          AND (title ILIKE ${'%' + search + '%'} OR artist ILIKE ${'%' + search + '%'}
               OR medium ILIKE ${'%' + search + '%'})
        ORDER BY synced_at DESC LIMIT ${lim} OFFSET ${off}
      `;
    } else if (search) {
      works = await sql`
        SELECT * FROM artworks
        WHERE commercial_ok = true AND thumb_url IS NOT NULL AND thumb_url != ''
          AND (title ILIKE ${'%' + search + '%'} OR artist ILIKE ${'%' + search + '%'}
               OR source ILIKE ${'%' + search + '%'} OR medium ILIKE ${'%' + search + '%'})
        ORDER BY synced_at DESC LIMIT ${lim} OFFSET ${off}
      `;
    } else if (source) {
      works = await sql`
